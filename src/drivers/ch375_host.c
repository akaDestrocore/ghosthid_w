    ...
    uint8_t *data, uint16_t wLength, int *actual_length, uint32_t timeout)
{
    ...
    if (wLength > 0) {
        while (total_received < wLength) {
            ...
            if (SETUP_IN(request_type)) {
                /* IN transfer */
                // LOG_DBG("Sending IN token (tog=%d, received=%d/%d)", tog, total_received, wLength);
                ...
                // LOG_DBG("Received %d bytes (total=%d/%d)", packet_len, total_received + packet_len, wLength);
                ...
                if (packet_len < udev->ep0_maxpack) {
                    // LOG_DBG("Short packet (%d < %d bytes), DATA stage complete", packet_len, udev->ep0_maxpack);
                    break;
                }
    ...
    // LOG_DBG("Control transfer complete: %d bytes transferred (requested %d)", total_received, wLength);
...
{
    if (!udev || !udev->raw_conf_desc || udev->raw_conf_desc_len == 0) {
        return CH375_HOST_ERROR;
    }

    uint8_t *ptr = (uint8_t *)udev->raw_conf_desc;
    uint8_t *end = ptr + udev->raw_conf_desc_len;
    ...
    while (ptr + sizeof(struct usb_desc_header) <= end) {
        ...
        /* sanity check length */
        if (hdr->bLength == 0) {
            LOG_ERR("Descriptor parsing error: zero length");
            return CH375_HOST_ERROR;
        }

        /* ensure hdr fits */
        if (ptr + hdr->bLength > end) {
            LOG_ERR("Descriptor parsing error: truncated descriptor (len=%u, rem=%u)",
                    hdr->bLength, (unsigned)(end - ptr));
            return CH375_HOST_ERROR;
        }

        switch (hdr->bDescriptorType) {
        case USB_DESC_INTERFACE:
            parse_interface_descriptor(udev, (struct usb_if_descriptor *)hdr);
            break;
    ...
    // LOG_DBG("Parsed config: interfaces=%d", udev->interface_cnt);
    return CH375_HOST_SUCCESS;
}
...
int ch375_host_udev_open(struct ch375_context *ctx, struct usb_device *udev)
{
    ...
    LOG_INF("Device descriptor: bMaxPacketSize0=%u, VID:PID=%04X:%04X",
        udev->raw_dev_desc.bMaxPacketSize0,
        sys_le16_to_cpu(udev->raw_dev_desc.idVendor),
        sys_le16_to_cpu(udev->raw_dev_desc.idProduct));
    ...
    LOG_INF("Short config: total_len=%u, configuration_value=%u",
        conf_total_len, short_conf_desc.bConfigurationValue);


    LOG_INF("Parsed config: interfaces=%d", udev->interface_cnt);
    for (int i = 0; i < udev->interface_cnt; ++i) {
        LOG_INF(" Interface %d: endpoints=%d class=0x%02X", i,
                udev->interface[i].endpoint_cnt, udev->interface[i].interface_class);
        for (int j = 0; j < udev->interface[i].endpoint_cnt; ++j) {
            LOG_INF("  EP[%d] addr=0x%02X attr=0x%02X maxpack=%d interval=%d tog=%d",
                    j,
                    udev->interface[i].endpoint[j].ep_num,
                    udev->interface[i].endpoint[j].attr,
                    udev->interface[i].endpoint[j].maxpack,
                    udev->interface[i].endpoint[j].interval,
                    udev->interface[i].endpoint[j].tog);
        }
    }
    ...
    LOG_HEXDUMP_DBG(udev->raw_conf_desc, udev->raw_conf_desc_len, "full config descriptor");
