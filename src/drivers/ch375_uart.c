...
typedef struct {
    ...
    /* Logging rate-limit / suppression state */
    uint16_t last_cmd;
    uint32_t cmd_repeat;
    uint32_t last_cmd_log_ts;

    uint16_t last_data;
    uint32_t data_repeat;
    uint32_t last_data_log_ts;

    uint16_t last_read_val;
    uint32_t read_repeat;
    uint32_t last_read_log_ts;
} ch375_hw_context_t;

static USART_TypeDef *get_uart_instance(const struct device *dev);

static void maybe_log_repeat(const char *fmt, uint32_t *last_ts, uint32_t interval_ms,
                             uint32_t *repeat_count, ...)
{
    uint32_t now = k_uptime_get_32();

    va_list ap;
    va_start(ap, repeat_count);

    if ((now - *last_ts) >= interval_ms) {
        if (*repeat_count > 0) {
            /* print summary of repeats then reset */
            // LOG_DBG("%s (repeated %u times)", fmt, *repeat_count);
            *repeat_count = 0;
        }
        *last_ts = now;
    } else {
        /* increment only - actual printing suppressed until interval expires */
        (*repeat_count)++;
    }
                                ...
                                k_timeout_t timeout)
{
    ...
    /* ... compute timeout_ms as in your original code ... */
    if (K_TIMEOUT_EQ(timeout, K_FOREVER)) {
        timeout_ms = INT64_MAX;
    } else if (K_TIMEOUT_EQ(timeout, K_NO_WAIT)) {
        timeout_ms = 0;
    ...
    *data = uart_instance->DR & 0x01FF;
    ...
    /* Only log when there was a non-zero number of polling attempts (diagnostic) */
    if (attempts > 0) {
        // LOG_DBG("RX success after %u attempts, took %lld ms, data=0x%04X",attempts, (k_uptime_get() - start_time), *data);
    }
...
static int ch375_write_cmd_cb(struct ch375_context *ctx, uint8_t cmd)
{
    ...
    /* Suppress flooding of identical commands: print once immediately, then
     * summarize repeats at ~200 ms granularity.
     */
    uint32_t now = k_uptime_get_32();
    if (hw->last_cmd == cmd) {
        hw->cmd_repeat++;
        if ((now - hw->last_cmd_log_ts) >= 200) {
            // LOG_DBG("%s: CMD 0x%04X (repeat %u)", hw->name, data, hw->cmd_repeat);
            hw->cmd_repeat = 0;
            hw->last_cmd_log_ts = now;
        }
    } else {
        hw->last_cmd = cmd;
        hw->cmd_repeat = 0;
...
static int ch375_read_data_cb(struct ch375_context *ctx, uint8_t *data)
{
    ...
    if (ret < 0) {
        if (ret == -ETIMEDOUT) {
            /* expected for short packets, keep it quiet */
    ...
    *data = (uint8_t)(val & 0xFF);
    ...
    uint32_t now = k_uptime_get_32();
    if (hw->last_read_val == val) {
        hw->read_repeat++;
        if ((now - hw->last_read_log_ts) >= 200) {
            // LOG_DBG("%s: READ 0x%04X -> 0x%02X (repeat %u)", hw->name, val, *data, hw->read_repeat);
            hw->read_repeat = 0;
            hw->last_read_log_ts = now;
        }
    } else {
        hw->last_read_val = val;
        hw->read_repeat = 0;
        hw->last_read_log_ts = now;
